...
int main(int argc, char *argv[]){
	...
	if(argc != 3){
		printf("Forma de uso: programa numero_registros nombre_del_archivo_final\n");
		exit(0);
	}

	archivoGuardar = open(argv[2], O_WRONLY|O_TRUNC|O_CREAT, 0666);
	...
	for(j=0; j<n; j++){
		...
		write(archivoGuardar, (char*)&reg1, sizeof(reg1));
		//write(archivoGuardar, '\0', sizeof(char));
		printf("\n");
	}
}
